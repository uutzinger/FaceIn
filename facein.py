...
# Sign In 
#
# Based on doorcam.py from Adam Geitgey
...
def running_on_jetson_nano():
    ...
    # when we are running on the Nano so that we can access the camera correctly 
    # in that case.
...
def update_recent_visitors_display(frame, time_history):
# Display recent visitor images

    (y_max, x_max) = frame.shape[:2]  # first two dimensions
    x_max = x_max -  1920
    frame.fill(0) # clear display frame
    # initialize
    ...
    for metadata in known_face_metadata:
        ...
        if datetime.now() - metadata["last_seen"] < timedelta(minutes=time_history) and metadata["seen_frames"] > 2:
            # Draw the known face image
            frame[y_position:y_positon+150, x_position:x_position+150] = metadata["face_image"]
            x_position += 180
            if x_position >= x_max: 
                x_position = 0
                y_position += 180                
            ...
            cv2.putText(frame, visit_label, (x_position + 10, y_position + 170), cv2.FONT_HERSHEY_DUPLEX, 0.6, (255, 255, 255), 1)
    return frame

def main_loop():

    # 4k display is 3840 x 2160
    # 1080p is 1920 x 1080
    x_max = 10*180 + 1920
    y_max = 15*180

    # Preallocate display image
    display_frame = np.zeros((y_max,x_max,3), np.uint8)
    ...
    while True:
        ...
        if (current_time - last_fps_time) >= 5.0: # update frame rate every 5 secs
            print("Frame Rate:{}[1/s], Processing Face:{}[%], Reading Camera:{}[%], Displaying Image:{}[%]".format(num_frames/5.0, int(total_proc_time*20.0),int(total_read_time/tmp_scale*20.0),int(total_disp_time/tmp_scale*20.0))
            num_frames = 0
            total_proc_time = 0.0
            total_read_time = 0.0
            total_disp_time = 0.0
        ...
        if capture.new_frame:
            ...
            if time.time() - last_display_update > 1:
                display_frame = update_recent_visitors_display(display_frame, 60)
                last_display_udate = time.time()

            display_frame[-frame_size(0):-1, -frame_size(1):-1, :] = frame
            cv2.imshow('Registration Cam', display_frame)
